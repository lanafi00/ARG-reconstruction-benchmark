...
if __name__ == "__main__":
    ...
    for demo_name in demos_to_run:
        run(demo_name)(/gpfs/data/steinruecken-lab/conda/lanaf/working_env) [t-9lanaf@cri22in001 my-singer-run]$ cat make_plots_step.py
"""
Compare ground-truth vs. SINGER-reconstructed pairwise coalescence statistics
(proportions, rates, counts) for the stepping-stone demographic model, with
and without Polegon post-processing, across all fifteen within-/cross-deme
index comparisons (5 demes -> 5 within-deme + 10 cross-deme pairs).

No closed-form Ne(t) is known for this model, so no analytic overlay is drawn
on the rates plot (matching the out-of-Africa script).
...
# Population metadata name tags for the 5 demes in this stepping-stone model,
# in sample_sets order [deme0, deme1, deme2, deme3, deme4].
# NOTE: truth and reconstruction pipelines label these demes completely
# differently (arbitrary pop_N IDs vs. D{row}_{col}-style grid coordinates) --
# this is expected, not a bug. Keep both lists in sync with however each
# pipeline currently names things.
TRUTH_DEME_METADATA_NAMES = ["pop_1", "pop_9", "pop_73", "pop_81", "pop_41"]
RECONSTRUCTION_DEME_METADATA_NAMES = ["D0_D0", "D0_D8", "D8_D0", "D8_D8", "D4_D4"]
...
POP_COMPARISON_INDEXES = [(0,0),(4,4),(0,4),(0,1),(0,3)]
POP_COMPARISON_LABELS = {
    (i, j): f"({DEME_LABELS[i]},{DEME_LABELS[j]})"
    for (i, j) in POP_COMPARISON_INDEXES
}
...
def _get_named_populations(ts: tskit.TreeSequence, deme_names: list[str]):
    """Return a list of tskit Population objects matched by metadata name, in `deme_names` order (None for missing)."""
    pops = [None] * len(deme_names)
    name_to_slot = {name: idx for idx, name in enumerate(deme_names)}
    for pop in ts.populations():
        metadata = pop.metadata
        if metadata and "name" in metadata and metadata["name"] in name_to_slot:
            pops[name_to_slot[metadata["name"]]] = pop
    return pops
...
def get_reconstructed_means_pop_comparison(pop_indexes, demo_name: str,
    ...
    """
    Average pair-coalescence counts/proportions/rates across all replicate
    reconstructions, for one deme index comparison (e.g. within-deme, or
    cross-deme).
    """
    results_dir = _results_dir_name(use_polegon)
    counts_list, props_list, rate_list = [], [], []
    ...
    for j in range(NUM_REPLICATES):
        ...
        pops = _get_named_populations(ts, RECONSTRUCTION_DEME_METADATA_NAMES)
        if None in pops:
            print(f"[{demo_name}] warning: missing population in tree {j}, skipping")
            n_skipped += 1
            continue
...
def get_truth_pop_comparison(pop_indexes, ts: tskit.TreeSequence, time_intervals: np.ndarray):
    """Ground-truth pair-coalescence counts/proportions/rates for one deme index comparison."""
    pops = _get_named_populations(ts, TRUTH_DEME_METADATA_NAMES)
    if None in pops:
        print("warning: missing population in ground-truth tree")
        return None, None, None
...
def plot_rates(demo_name, pop_comparison_label, plot_intervals,
    ...
    ax.stairs(polegon_rate[1:-1], plot_intervals[1:-1], color="red", linewidth=1.5, label="SINGER + Polegon")
    ax.stairs(no_polegon_rate[1:-1], plot_intervals[1:-1], color="green", linewidth=1.5, label="SINGER")
    # No closed-form Ne(t) for stepping-stone, so no "Empirical Ne(t)" overlay here.
    ax.set_xscale("log")
    ax.set_xlabel("Generations in past")
    ax.set_ylabel("Pair coalescent rates")
    ax.legend()
...
if __name__ == "__main__":
    demos_to_run = ["step_005","step_0005","step_05"]
